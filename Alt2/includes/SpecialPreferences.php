...
class PreferencesForm {
	...
	function mainPrefsForm( $status , $message = '' ) {
		...
		$userInformationHtml =
			...
			$this->tableRow('Hometown', htmlspecialchars( $wgUser->getCity() ) ) . // added by chris russo
			$this->tableRow('Citizen Status', 'fucking god son') . // added by chris russo
		...
		if ( $wgEnableEmail ) {
			...
			if ($wgEnableUserEmail) {
				...
				//*********** THIS NEEDS TO BE CHANGED BUT FOR NOW  ********************/
				$newone = "<input type='checkbox' checked='checked' value='1' name='whatev' id='whatev' /> <label for='whatev'> 
							what about event emails or some other shit... lets default this to on </label><br />";
			$wgOut->addHTML(
